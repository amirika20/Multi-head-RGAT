...
def train(
    ...
    for epoch, batch_data in enumerate(dataloader):  # single graph batch
        ...
        if (epoch + 1) % 500 == 0:
            ...
            node_embedding, edge_embedding = model.get_embedding(test_g, test_nids, device)
            mrr = calc_mrr(
                model, node_embedding, edge_embedding, test_mask, triplets, batch_size=500
            )
            # save best model
            if best_mrr < mrr:
                best_mrr = mrr
...
if __name__ == "__main__":
    ...
    # create RGCN model
    model = LinkPredict(14541, 474, 128, 128, 128, 128, 8, 3, self_loop=False).to(device)
    model.to(device)
    # train
