...
def perturb_and_get_filtered_rank(
    ...
    for idx in tqdm.tqdm(range(test_size), desc="Evaluate"):
        ...
        #print(target_s.expand((num_triplets,1)).shape)
        #print(target_r.expand((num_triplets,1)).shape)
        #print(target_o.expand((num_triplets,1)).shape)
        #print(candidate_nodes.shape)
        if filter_o:
            triplets = torch.cat([target_s.expand((num_triplets,1)),target_r.expand((num_triplets,1)), candidate_nodes.unsqueeze(dim=1)],dim=1)
        else:
            triplets = torch.cat([candidate_nodes.unsqueeze(dim=1),target_r.expand((num_triplets,1)), target_o.expand((num_triplets,1))],dim=1)
        target_idx = 0
